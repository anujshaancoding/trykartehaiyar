...
function BlogDetail() {
  ...
  useMetaTags({
    ...
    description: blog?.[language]?.excerpt,
  ...
  if (!blog) {
    return (
      <div className="blog-page blog-not-found">
        <h1>{language === 'hi' ? 'ब्लॉग नहीं मिला' : 'Blog Not Found'}</h1>
        <Link to="/blog" className="blog-back-btn">
          {language === 'hi' ? 'सभी ब्लॉग देखें' : 'View All Blogs'}
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="blog-page blog-detail-page">
      ...
      <nav className="blog-navigation">
        {prevBlog && (
          <Link to={`/blog/${prevBlog.slug}`} className="blog-nav-link blog-nav-prev">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M19 12H5M12 19l-7-7 7-7" />
            </svg>
        ...
        {nextBlog && (
          <Link to={`/blog/${nextBlog.slug}`} className="blog-nav-link blog-nav-next">
            <div className="blog-nav-text">
